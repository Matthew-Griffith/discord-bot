...
bot.on("message", (message) => {
    ...
    */
    ...
    /*
      ...
      TODO: sometimes I am getting promise errors but I think I fixed them however, 
            I am not sure what is causing them in the first place. it might be here or
            in the !yt command. 
    */
    if (message.content.startsWith(prefix + "roll")) {
      var user_input = message.content.slice(6, message.content.length);
      var dice = {
      ...
      if (user_input.toLowerCase() === 'coin') {
        if (getRandomInt(0, 1) === 1) {
          message.channel.sendMessage('It is... Tails');
        }
        else {message.channel.sendMessage('It is... Heads');}
      }
      else if (dice[user_input]) {
        message.channel.sendMessage(dice[user_input]);
      }
